...
export function monthBarChart(container, series) {
  ...
  series.forEach((p, i) => {
    ...
    bars += `
      <g>
        ...
        <rect x="${cx - barW - 2}" y="${yV}" width="${barW}" height="${padT + innerH - yV}" rx="3.5" fill="#12b886"/>
        <rect x="${cx + 2}" y="${yU}" width="${barW}" height="${padT + innerH - yU}" rx="3.5" fill="#f2c879"/>
        <text x="${cx}" y="${H - 9}" text-anchor="middle" font-size="10.5" fill="var(--text-3)">${esc(p.label)}</text>
      </g>`;
  });
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    grid += `
      ...
      <text x="${padL - 8}" y="${gy + 3.5}" text-anchor="end" font-size="10.5" fill="var(--text-3)">${compact(v)}</text>`;
  }
