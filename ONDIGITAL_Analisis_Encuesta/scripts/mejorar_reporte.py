...
# Paleta oficial ONDIGITAL "Pulso Vital" (ver AGENTS.md y LEEME.txt).
# Los valores van sin "#" porque openpyxl usa ARGB/RRGGBB.
INK_DEEP = "0B1410"       # tinta verdigris profunda (portada, cabeceras oscuras)
INK_PANEL = "12201A"      # superficie verdigris
PARCHMENT = "F2EFE4"      # pergamino (texto sobre oscuro)
ACCENT = "8C6A2A"         # laton variante AA sobre claro (4.99:1 sobre blanco)
ACCENT_ALT = "6C35ED"     # violeta variante AA sobre claro (6.23:1 sobre blanco)
BRASS = "D8A24A"          # laton de marca (solo sobre oscuro: 7.37:1 sobre 12201A)
OK = "2FA277"             # verde verdigris para escalas de valor alto
WARN = "E0A83D"
DANGER = "E85D4E"
INK = "0B1410"            # texto principal
MUTED = "5F5B4E"          # texto secundario calido (6.79:1 sobre blanco)
LINE = "DBD5C4"           # linea/borde pergamino
PALE_ACCENT = "F7EEDC"    # tinte laton
PALE_OK = "E4F3EC"        # tinte verdigris
PALE_WARN = "FBF0D8"
PALE_DANGER = "FBE4E1"
ZEBRA = "FAF8F1"          # fila alterna
BORDER_ACCENT = "D9C49B"
BORDER_OK = "B4D9C7"
BORDER_WARN = "E8CE95"
WARN_TEXT = "6E5320"
WARN_TEXT_SOFT = "5A4419"
TAB_ALT = "6C35ED"
WHITE = "FFFFFF"
...
# La columna derivada segmento_comercial se guardo sin tildes en el CSV de datos.
# El dato no se toca; solo se normaliza la etiqueta que ve la persona usuaria.
SEGMENT_DISPLAY = {"Interes medio / diagnostico": "Interés medio / diagnóstico"}


def segment_display(value: str) -> str:
    return SEGMENT_DISPLAY.get(value, value)
        ...
        output.append(
            {
                ...
                "segmento": segment_display(row[headers[19]]),
            }
        )
    return output
...
def set_title(ws, title: str, subtitle: str, end_column: int = 14) -> None:
    ...
    ws["A1"] = title
    ws["A1"].font = Font(name="Fraunces", size=24, bold=True, color=WHITE)
    ws["A1"].fill = fill(INK_DEEP)
    ws["A1"].alignment = Alignment(vertical="center")
    ws.row_dimensions[1].height = 42
    ws.merge_cells(f"A2:{end}2")
    ws["A2"] = subtitle
    ws["A2"].font = Font(name="Inter", size=11, color=MUTED, italic=True)
    ws["A2"].alignment = Alignment(wrap_text=True, vertical="center")
    ws.row_dimensions[2].height = 28
    ws.sheet_view.showGridLines = False


def style_header(row, color: str = INK_DEEP) -> None:
    for cell in row:
...
def build_cover(wb: Workbook, stats: dict) -> None:
    ...
    ws["A5"].font = Font(name="Fraunces", size=22, bold=True, color=INK)
    ws["A5"].alignment = Alignment(wrap_text=True, vertical="center")
    ws["A5"].fill = fill(PALE_ACCENT)
    ws["A5"].border = border(BORDER_ACCENT)
    ...
    ws["A9"].font = Font(size=12, bold=True, color=ACCENT)
    ...
    ws["E9"].font = Font(size=12, bold=True, color=ACCENT)
    takeaways = [
    ...
    for row_index, (number, label) in enumerate(takeaways, 10):
        ...
        ws.cell(row_index, 5).font = Font(size=16, bold=True, color=ACCENT)
        ws.cell(row_index, 6).font = Font(size=10, color=INK)
        ws.cell(row_index, 5).fill = fill(PALE_OK)
        ws.cell(row_index, 6).fill = fill(PALE_OK)
        ws.cell(row_index, 5).border = border(BORDER_OK)
        ws.cell(row_index, 6).border = border(BORDER_OK)
    ...
    ws["A17"].font = Font(size=12, bold=True, color=WARN_TEXT)
    ...
    ws["A18"].fill = fill(PALE_WARN)
    ws["A18"].font = Font(size=10, color=WARN_TEXT_SOFT)
    ws["A18"].alignment = Alignment(wrap_text=True, vertical="center")
    ws["A18"].border = border(BORDER_WARN)

    ws["A23"] = "Navegación del paquete"
    ws["A23"].font = Font(size=12, bold=True, color=ACCENT)
    navigation = [
    ...
    for row_index, (sheet, description) in enumerate(navigation, 24):
        ...
        ws.cell(row_index, 1).font = Font(bold=True, color=ACCENT)
    ...
    ws.freeze_panes = "A9"
    ws.sheet_properties.tabColor = BRASS
...
def build_dashboard(wb: Workbook, stats: dict, metrics: dict) -> None:
    ...
    ws["A4"].fill = fill(PALE_ACCENT)
    ws["A4"].alignment = Alignment(wrap_text=True, vertical="center")
    ws["A4"].border = border(BORDER_ACCENT)
    ws.row_dimensions[4].height = 32

    ws["A6"] = "Indicadores de la muestra"
    ws["A6"].font = Font(size=12, bold=True, color=ACCENT)
    metric_rows = [
    ...
    ws["E6"].font = Font(size=12, bold=True, color=ACCENT)
    for column, value in enumerate(["Segmento", "n", "%"], 5):
        ws.cell(7, column, value)
    style_header(ws[7][4:7])
    segment_order = ["Listo para vender", "Interés medio / diagnóstico", "Sensible al precio", "Educar y nutrir", "Baja prioridad"]
    ...
    ws["I6"].font = Font(size=12, bold=True, color=ACCENT)
    quality = [
    ...
    ws["A34"].font = Font(size=12, bold=True, color=ACCENT)
    priorities = [
    ...
    ws["E34"].font = Font(size=12, bold=True, color=ACCENT)
...
def build_decisions(wb: Workbook, stats: dict) -> None:
    ...
    ws["A15"].font = Font(size=12, bold=True, color=ACCENT)
    script = [
    ...
    for row_index, (step, copy) in enumerate(script, 16):
        ...
        ws.cell(row_index, 1).font = Font(bold=True, color=ACCENT)
        ws.cell(row_index, 2).font = Font(color=INK)
        ws.cell(row_index, 1).fill = fill(PALE_OK)
        ws.cell(row_index, 2).fill = fill(PALE_OK)
        ws.cell(row_index, 1).border = border(BORDER_OK)
        ws.cell(row_index, 2).border = border(BORDER_OK)
        ws.cell(row_index, 2).alignment = Alignment(wrap_text=True)
    ws["A22"] = "Límite de interpretación"
    ws["A22"].font = Font(size=12, bold=True, color=WARN_TEXT)
    ...
    ws["A23"].fill = fill(PALE_WARN)
    ws["A23"].font = Font(color=WARN_TEXT_SOFT)
    ws["A23"].alignment = Alignment(wrap_text=True, vertical="center")
    ws["A23"].border = border(BORDER_WARN)
...
def build_responses(wb: Workbook, rows: list[dict]) -> None:
    ...
    ws.conditional_formatting.add(f"V5:V{4 + len(rows)}", ColorScaleRule(start_type="min", start_color=DANGER, mid_type="percentile", mid_value=50, mid_color=WARN, end_type="max", end_color=OK))
    ws.conditional_formatting.add(f"X5:X{4 + len(rows)}", FormulaRule(formula=['$X5="Listo para vender"'], fill=fill(PALE_OK)))
    ws.conditional_formatting.add(f"X5:X{4 + len(rows)}", FormulaRule(formula=['$X5="Baja prioridad"'], fill=fill(PALE_DANGER)))
    ...
    ws.sheet_properties.tabColor = TAB_ALT
    ws.page_setup.orientation = "landscape"
    ws.page_setup.fitToWidth = 1
    ws.sheet_properties.pageSetUpPr.fitToPage = True
...
def write_csv_sheet(ws, title: str, rows: list[list[str]], tab_color: str) -> None:
    ...
    for row_index, values in enumerate(rows, start_row):
        for column, value in enumerate(values, 1):
            ws.cell(row_index, column, value)
    if rows:
        style_header(ws[start_row], INK_DEEP)
...
def build_frequencies(wb: Workbook) -> None:
    ...
    write_csv_sheet(wb.create_sheet("04_Frecuencias"), "Frecuencias completas", rows, WARN)
...
def build_crosses(wb: Workbook, metrics: dict) -> None:
    ...
    for row_index, values in enumerate(assoc, 6):
        for column, value in enumerate(values, 1):
            ...
            if row_index > 6 and column == 3:  # Efecto: numérico para que la escala de color funcione
                cell.value = float(value)
                cell.number_format = "0.000"
            elif row_index > 6 and column == 5:  # n
                cell.value = int(value)
    style_header(ws[6], INK_DEEP)
    style_body(ws, 7, 5 + len(assoc) - 1, 6)
    add_table(ws, "TblAsociaciones", f"A6:F{5 + len(assoc)}")
    ws.conditional_formatting.add(f"C7:C{5 + len(assoc)}", ColorScaleRule(start_type="min", start_color=PALE_DANGER, mid_type="percentile", mid_value=50, mid_color=PALE_WARN, end_type="max", end_color=PALE_OK))

    for row_index in range(7, 5 + len(assoc) + 1):
        ws.row_dimensions[row_index].height = 46

    ws["H4"] = "Control de calidad"
    ws["H4"].font = Font(size=12, bold=True, color=ACCENT)
    quality = [
        ("Respuestas abiertas con plantilla", metrics["calidad_datos"]["q10_parseable_misma_plantilla"]),
        ("Pares en el mismo segundo", metrics["calidad_datos"]["intervalos_consecutivos_mismo_segundo"]),
    ...
    for row_index, (label, value) in enumerate(quality, 6):
        ws.cell(row_index, 8, label)
        ws.cell(row_index, 9, value)
        ws.cell(row_index, 8).font = Font(color=MUTED, size=10)
        ws.cell(row_index, 9).font = Font(color=INK, bold=True)
        ws.cell(row_index, 8).border = border()
        ws.cell(row_index, 9).border = border()
    ws["H13"] = "Cómo leer esta tabla"
    ws["H13"].font = Font(size=12, bold=True, color=ACCENT)
    ws.merge_cells("H14:M20")
    ws["H14"] = ("Todas las filas se calculan sobre las mismas n = 321 respuestas. La V de Cramér (corregida por sesgo) mide fuerza de asociación entre variables "
        "nominales y nunca es negativa; la rho de Spearman mide la tendencia monótona y sí lleva signo, por eso madurez vs disposición aparece dos veces con "
        "valores distintos y no es una contradicción. La asociación más fuerte del archivo es equipo externo vs aprendizaje (rho = 0.638). "
        "La fila de almacenamiento vs percepción de pérdida no cumple el supuesto de chi-cuadrado (20 de 35 celdas con frecuencia esperada < 5): no citarla como resultado significativo. "
        "Nada de esto prueba causalidad ni representatividad del mercado.")
    ws["H14"].fill = fill(PALE_ACCENT)
    ws["H14"].alignment = Alignment(wrap_text=True, vertical="center")
    ws["H14"].border = border(BORDER_ACCENT)
    set_widths(ws, {"A": 40, "B": 32, "C": 10, "D": 12, "E": 6, "F": 62, "G": 4, "H": 34, "I": 16, "J": 16, "K": 16, "L": 16, "M": 16})
    ws.freeze_panes = "A7"
    ws.sheet_properties.tabColor = DANGER
    ws.page_setup.orientation = "landscape"
...
def build_dictionary(wb: Workbook) -> None:
    ...
    ws.freeze_panes = "A5"
    ws.sheet_properties.tabColor = WARN
    ws.page_setup.orientation = "landscape"
...
<style>
:root{--bg:#0B1410;--panel:#12201A;--panel-2:#16241E;--ink:#F2EFE4;--muted:#C8C2AE;--line:rgba(242,239,228,.16);--parchment:#F2EFE4;--accent:#D8A24A;--accent-2:#9B8CFF;--ok:#4FC48D;--warn:#E0A83D;--danger:#E85D4E;--ink-deep:#0B1410;--ink-panel:#12201A;--shadow:0 18px 48px rgba(0,0,0,.30);--radius:16px;--font:Inter,"Inter var","Segoe UI",Roboto,ui-sans-serif,system-ui,-apple-system,"Helvetica Neue",Arial,sans-serif;--mono:"JetBrains Mono","JetBrainsMono Nerd Font","SFMono-Regular",Consolas,"Liberation Mono",ui-monospace,monospace;--display:Fraunces,"Fraunces 9pt",Georgia,"Times New Roman",ui-serif,serif}
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background-color:var(--bg);background-image:linear-gradient(rgba(140,106,42,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(140,106,42,.05) 1px,transparent 1px);background-size:32px 32px;color:var(--ink);font:14px/1.5 var(--font)}a{color:inherit}button,input,select{font:inherit}button{cursor:pointer}button:focus-visible,a:focus-visible,input:focus-visible,select:focus-visible{outline:3px solid color-mix(in srgb,var(--accent) 70%,transparent);outline-offset:3px}.shell{width:min(1440px,calc(100% - 32px));margin:auto}.topbar{position:sticky;top:0;z-index:20;background:color-mix(in srgb,var(--bg) 90%,transparent);border-bottom:1px solid var(--line);backdrop-filter:blur(16px)}.topbar-inner{min-height:64px;display:flex;align-items:center;justify-content:space-between;gap:14px}.brand{display:flex;align-items:center;gap:11px;font-weight:850;letter-spacing:.12em;font-size:13px}.mark{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,var(--accent-2),var(--accent));color:var(--ink-deep);font-weight:950;letter-spacing:-.08em}.top-actions{display:flex;gap:8px;align-items:center}.btn{border:1px solid var(--line);background:var(--panel);color:var(--ink);border-radius:10px;padding:9px 12px;font-weight:750;text-decoration:none;transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}.btn:hover{transform:translateY(-1px);border-color:var(--accent-2);box-shadow:0 8px 22px rgba(140,106,42,.18)}.btn.primary{background:var(--accent);color:var(--ink-deep);border-color:var(--accent)}
.page-head{padding:48px 0 28px;display:flex;align-items:end;justify-content:space-between;gap:24px}.kicker{font:700 11px/1 var(--mono);text-transform:uppercase;letter-spacing:.16em;color:var(--accent);margin-bottom:12px}.page-head h1{font-size:clamp(34px,5vw,66px);line-height:.95;letter-spacing:-.05em;margin:0}.page-head p{max-width:640px;color:var(--muted);margin:13px 0 0;font-size:16px}.page-head .head-note{max-width:310px;border-left:1px solid var(--accent);padding-left:14px;color:var(--muted);font-size:12px}.control-panel{display:grid;grid-template-columns:minmax(240px,2fr) repeat(2,minmax(170px,1fr)) auto;gap:10px;align-items:end;padding:16px;border:1px solid color-mix(in srgb,var(--accent-2) 25%,var(--line));border-radius:var(--radius);background:var(--panel);box-shadow:var(--shadow)}.field{display:flex;flex-direction:column;gap:6px}.field label{font:700 10px/1 var(--mono);letter-spacing:.08em;text-transform:uppercase;color:var(--muted)}.field input,.field select{width:100%;min-width:0;border:1px solid var(--line);border-radius:10px;background:var(--bg);color:var(--ink);padding:10px 11px}.field input:hover,.field select:hover{border-color:var(--accent-2)}.control-actions{display:flex;gap:8px;flex-wrap:wrap}.result-bar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin:13px 0;color:var(--muted);font-size:12px}.result-bar strong{color:var(--ink);font-size:15px}.result-bar .status{display:inline-flex;align-items:center;gap:8px}.status i{width:8px;height:8px;border-radius:50%;background:var(--accent);box-shadow:0 0 0 4px color-mix(in srgb,var(--accent) 14%,transparent)}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:16px 0 26px}.stat{border:1px solid var(--line);border-radius:14px;background:var(--panel);padding:15px}.stat b{display:block;font-size:26px;letter-spacing:-.05em;color:var(--accent)}.stat span{display:block;color:var(--muted);font-size:11px;margin-top:4px}.table-card{border:1px solid var(--line);border-radius:var(--radius);background:var(--panel);box-shadow:var(--shadow);overflow:hidden}.table-head{display:flex;align-items:end;justify-content:space-between;gap:16px;padding:20px 20px 15px;border-bottom:1px solid var(--line)}.table-head h2{font-size:20px;letter-spacing:-.02em;margin:0}.table-head p{color:var(--muted);margin:4px 0 0;font-size:12px}.table-tools{display:flex;gap:8px;flex-wrap:wrap}.table-scroll{overflow:auto;max-height:650px}.table-scroll table{border-collapse:collapse;min-width:1320px;width:100%;font-size:12px}.table-scroll th,.table-scroll td{padding:11px 12px;border-bottom:1px solid var(--line);vertical-align:top;text-align:left}.table-scroll th{position:sticky;top:0;z-index:2;background:var(--panel-2);color:var(--muted);font:700 10px/1.2 var(--mono);text-transform:uppercase;letter-spacing:.04em}.table-scroll tbody tr{transition:background .16s ease}.table-scroll tbody tr:hover,.table-scroll tbody tr:focus{background:color-mix(in srgb,var(--accent-2) 9%,var(--panel));outline:none}.table-scroll td:first-child,.table-scroll th:first-child{position:sticky;left:0;z-index:3;background:var(--panel);font-family:var(--mono);color:var(--muted)}.table-scroll th:first-child{background:var(--panel-2)}.wrap{white-space:normal;min-width:220px;max-width:310px}.score{font-weight:850;color:var(--accent);font-family:var(--mono)}.segment{display:inline-flex;border-radius:999px;padding:5px 8px;background:color-mix(in srgb,var(--accent-2) 13%,transparent);color:var(--accent-2);font-size:10px;font-weight:800;white-space:nowrap}.pagination{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 20px;color:var(--muted);font-size:12px}.pagination-actions{display:flex;gap:8px}.btn:disabled{cursor:not-allowed;opacity:.45;transform:none;box-shadow:none}.empty{padding:42px 18px;text-align:center;color:var(--muted)}dialog{width:min(720px,calc(100% - 32px));max-height:90vh;overflow:auto;border:1px solid var(--line);border-radius:16px;background:var(--panel);color:var(--ink);padding:0;box-shadow:0 25px 80px rgba(0,0,0,.45)}dialog::backdrop{background:rgba(11,20,16,.72);backdrop-filter:blur(5px)}.dialog-head{display:flex;justify-content:space-between;gap:12px;padding:18px 20px;border-bottom:1px solid var(--line)}.dialog-head h2{font-size:19px;margin:0}.dialog-head p{color:var(--muted);font:11px var(--mono);margin:5px 0 0}.dialog-body{padding:18px 20px}.detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.detail{border:1px solid var(--line);border-radius:12px;padding:12px;background:var(--panel-2)}.detail label{display:block;color:var(--muted);font:700 10px/1.2 var(--mono);text-transform:uppercase;margin-bottom:5px}.detail strong{font-size:13px}.detail.full{grid-column:1/-1}.footer{padding:34px 0 60px;color:var(--muted);font-size:11px}.footer strong{color:var(--ink)}
...
:root{--bg:#F4F1E6;--panel:#FFFDF8;--panel-2:#EFEBDD;--ink:#0B1410;--muted:#5F5B4E;--line:#DBD5C4;--parchment:#F2EFE4;--accent:#8C6A2A;--accent-2:#6C35ED;--ok:#1C7A55;--warn:#8A6400;--danger:#B4392B;--ink-deep:#0B1410;--ink-panel:#12201A;--shadow:0 10px 30px rgba(43,36,20,.10);--radius:10px}
body.dark{--bg:#0B1410;--panel:#12201A;--panel-2:#16241E;--ink:#F2EFE4;--muted:#C8C2AE;--line:rgba(242,239,228,.16);--parchment:#F2EFE4;--accent:#D8A24A;--accent-2:#9B8CFF;--ok:#4FC48D;--warn:#E0A83D;--danger:#E85D4E;--ink-deep:#070D09;--ink-panel:#16241E;--shadow:0 14px 36px rgba(0,0,0,.34)}
...
.mark{width:32px;height:32px;border-radius:7px;background:var(--ink-deep);color:var(--parchment);border-bottom:3px solid var(--accent);font-size:11px}
...
.btn:hover{transform:none;background:var(--panel-2);border-color:var(--accent-2);box-shadow:none}
.btn.primary{background:var(--ink-deep);color:var(--parchment);border-color:var(--ink-deep)}
.btn.primary:hover{background:var(--ink-panel);border-color:var(--ink-panel)}
button:focus-visible,a:focus-visible,input:focus-visible,select:focus-visible{outline:3px solid color-mix(in srgb,var(--accent-2) 42%,transparent);outline-offset:2px}
...
.kicker{margin-bottom:10px;color:var(--accent);font:800 12px/1.2 var(--mono);letter-spacing:.06em;text-transform:uppercase}
...
.page-head .head-note{max-width:none;padding:14px 16px;border:0;border-left:3px solid var(--accent);border-radius:0 8px 8px 0;background:var(--panel);color:var(--muted);font-size:12px}
...
.status i{background:var(--accent);box-shadow:none}
...
.stat b{color:var(--accent);font-size:27px}
.stat span{font-size:11px}
.table-card{border-color:var(--line);border-radius:10px;background:var(--panel);box-shadow:0 2px 10px rgba(43,36,20,.06)}
...
.table-scroll tbody tr:hover,.table-scroll tbody tr:focus{background:color-mix(in srgb,var(--accent-2) 7%,var(--panel))}
.table-scroll td:first-child{background:var(--panel)}.table-scroll th:first-child{background:var(--panel-2)}
.segment{border-radius:5px;background:color-mix(in srgb,var(--accent-2) 10%,transparent);color:var(--accent-2)}
...
<main class="shell">
  ...
  <section class="control-panel" aria-label="Filtros de respuestas">
    ...
    <div class="field"><label for="segment">Segmento</label><select id="segment"><option value="">Todos los segmentos</option><option>Listo para vender</option><option>Educar y nutrir</option><option>Sensible al precio</option><option>Interés medio / diagnóstico</option><option>Baja prioridad</option></select></div>
